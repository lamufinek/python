...
class bullet():
    ...
    def move(self):
        ...
        x = accPosition[0] + self.velX
        y = accPosition[1] + self.velY
...
director.init(width=700, height=650, autoscale=False, resizable=True)
...
class GameAction(Action, RectMapCollider):
    ...
    def step(self, dt):
        ...
        dy = dy + self.acc* self.timer+ self.flyAcc* self.timer# * dt
...
class GameActionEnemy(Action, RectMapCollider):
...
class GameActionEnemyRevers(Action, RectMapCollider):
...
class Enemy2():
    def __init__(self,x,y):
        image = pyglet.image.load("resources/monster2.png")
        image_gride = pyglet.image.ImageGrid(image,2,10,item_width=139,item_height=145)
        self.animation2 = pyglet.image.Animation.from_image_sequence(image_gride[10:15],0.1,True)
        self.animation = pyglet.image.Animation.from_image_sequence(image_gride[15:20],0.1,True)    
        self.sprite = Sprite(self.animation)
        self.sprite.position = x,y
        self.sprite.cshape = cm.AARectShape(
                        self.sprite.position,
                ...
                self.sprite.height/3)
        GameActionEnemy()
        self.sprite.do(GameActionEnemy())
        self.timer =0
        self.hp = 10
        self.hittedTimer = 10
    ...
    def updateEnemy(self):
        ...
        if(self.timer<150 and self.sprite.image != self.animation ):
            self.sprite.image = self.animation

            
        elif(self.timer>150 and self.sprite.image != self.animation2):
            self.sprite.image =self.animation2
...
class SpriteLayer(ScrollableLayer):
    ...
    def update(self,dt):
        for each in self.ogrs:
            each.updateEnemy()
        if(self.counter ==0 or self.counter==2):
            if(self.kierunek == "prawo" or self.kierunek =="prawogora"):
                self.sprite.image = self.animationStopPrawo
            else:
                self.sprite.image = self.animationStopLewo

        if(self.keyPressed):
            self.sprite.velocity = 0,0


        for nietoperz in self.nietoperki:
            nietoperz.updateEnemy()
            for pocisk in self.bullets:
                zmienna = self.collision_manager.they_collide(pocisk.returnSprite(),
                                                    nietoperz.returnSprite())
                if(zmienna):
                    super().remove(pocisk.returnSprite())
                    self.collision_manager.remove_tricky(pocisk.returnSprite())
                    self.bullets.remove(pocisk)
                    super().remove(nietoperz.returnSprite())
                    self.collision_manager.remove_tricky(nietoperz.returnSprite())
                    self.nietoperki.remove(nietoperz)

        for ogr in self.ogrs:
            ogr.updateEnemy()
            for pocisk in self.bullets:
                zmienna = self.collision_manager.they_collide(pocisk.returnSprite(),
                                                    ogr.returnSprite())
                if(zmienna):
                    ...
                    self.bullets.remove(pocisk)
                    if(ogr.hp<0):
                        super().remove(ogr.returnSprite())
                        self.collision_manager.remove_tricky(ogr.returnSprite())
                        self.ogrs.remove(ogr)
    ...
    def on_key_press(self, symbol, modifiers):
        if(symbol == key.Z):
            self.shoot()
        if(symbol == key.RIGHT):
            self.sprite.image=self.animationRight
            self.counter += 1
            self.kierunek="prawo"
        if(symbol == key.LEFT):
        ...
        if(symbol == key.UP):
            if(self.kierunek=="prawo"):
                self.kierunek="prawogora"
            if(self.kierunek=="lewo"):
                self.kierunek="lewogora"

 
    def on_key_release(self,symbol,modifiers):
        if(symbol==key.RIGHT or symbol==key.LEFT):
            self.counter -= 1
        if(symbol==key.UP and self.kierunek=="lewogora"):
            self.kierunek="lewo"
        if(symbol==key.UP and self.kierunek=="prawogora"):
            self.kierunek="prawo"
    ...
    def shoot(self):
        ...
        accBullet = bullet(self.sprite.position, self.kierunek)

        if(self.kierunek == "prawo"):
            accBullet.returnSprite().rotation = 0
        elif(self.kierunek == "lewo"):
            accBullet.returnSprite().rotation = 180
        elif(self.kierunek == "lewogora"):
            accBullet.returnSprite().rotation = -135
        elif(self.kierunek == "prawogora"):
            accBullet.returnSprite().rotation = -45
